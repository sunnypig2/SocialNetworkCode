...
def make_file(node,edges):
    if os.path.exists("output.gml"):
        os.remove("output.gml")

    fp = open("output.gml","a")
    fp.write('Creator "Mark Newman on Fri Jul 21 12:44:53 2006"'+'\n')
    fp.write('graph'+'\n')
    fp.write('['+'\n')
    ...
    for i in range(len(node)):
        ...
        fp.write('['+'\n')
        fp.write('id ' + str(i) + '\n')
        fp.write('label "' + node[i] + '"' + '\n')
        fp.write(']'+'\n')

    for edge in edges:
        fp.write('edge'+'\n')
        fp.write('['+'\n')
        fp.write('source ' + str(dict[edge[0]])+'\n')
        fp.write('target ' + str(dict[edge[1]])+'\n')
        fp.write('value ' + str(1)+'\n')
        fp.write(']'+'\n')
